...
export const createAuthSlice: SliceCreator<
  Pick<State, 'bootAuth' | 'signIn' | 'signOut' | 'completePasswordSetup' | 'enterLocalOnly' | 'leaveLocalOnly'>
> = (set, get, ctx) => {
  ...
  const maybeAutoReconnect = async (): Promise<void> => {
    ...
    if (!isCollabReady()) return; // persisted intent + env + 방 코드 전제
  ...
  const onAuthEvent = (e: AuthEvent): void => {
    if (e.kind === 'signed-out') {
      ...
      // ⚠️ persisted na_collab_enabled intent 는 지우지 않는다 — 다음 로그인의 자동 재접속을 위해 보존.
  ...
  return {
    ...
    leaveLocalOnly: async () => {
      dropLs(NA_LOCAL_ONLY);
      // ⚠️ na_collab_enabled intent 는 지우지 않는다 — 로그인 후 자동 재접속을 위해 보존한다.
