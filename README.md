...
- **배경**은 `transform vn_bg` 가 `fit "cover"` 로 화면을 꽉 채운다 — 소스가 3:2(1216×832)라 16:9 게임에선 위아래가 다소 잘리지만 **검은 띠·왜곡 없이** 채워진다(업로드한 어떤 크기 이미지든 동일).
...
- 좌측 **🏞️ 랜덤 배경 생성 ▶** — DB 의 장소(Scene) 태그 1개를 핵심으로 시간대·날씨·조명·톤을 무작위로 입혀 **인물 없는 배경**을 1장씩 무료로 생성(공식 가로 `1216×832` · Anlas 0). `no humans` + 인물 차단 네거티브로 사람이 끼지 않게 한다. 결과는 세션 메모리 + 보관 폴더 `random-bg/` 에 쌓인다.
- 각 썸네일 hover: **📋**(프롬프트+시드 복사) · **⬆ 업스케일 4×**. 배경 당첨작은 장면/에셋의 배경 **↥ 업로드**로 적용.
