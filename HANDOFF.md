...
## 🎯 다음 할 일
- **복장·표정 자동 추론(LLM) 도입 — Phase 단위 진행 중. Phase 5(표정 AI 문맥 품질 = F2/F3) 구현 완료, GPT 검토 → 확정 대기.** 규칙·Phase 로그·확정 설계는 전부 [`PHASES.md`](./PHASES.md) 에 있다(Claude 계획 → GPT 검토 → 구현 → 검토 → 확정 루프).
  - 확정되면 `PHASES.md` 로그 표 Phase 5 행(✅ + 커밋 해시)과 확정 설계 절을 채운다. 그 다음 Phase 프롬프트는 사용자가 준다 — **미리 시작하지 말 것.**
  - 검증 결과: typecheck · vitest 42파일/549 · `.rpy` 22구성 245파일 회귀 0(`dump:rpy` before/after `diff -r`) · 스크래치 outDir 빌드. **표정 품질 자체는 실키 검증이 연기 상태라 증명되지 않았다**(구조적으로 문맥이 실린다는 것까지만 테스트로 증명).
...
- **Phase 5 구현** — 표정 AI가 이제 주인공 대사·지문·이미 배정된 줄·직전 청크 대사를 읽기 전용 문맥으로 본다(`planEmotionChunks` 단일 소스, 요청당 60줄/2000자 상한). 옛 `prevContext`(직전 target 3줄)는 삭제·흡수.
