...
## 🎯 다음 할 일
- **복장·표정 자동 추론(LLM) 도입 — Phase 단위 진행 중. Phase 4 확정 완료, 다음은 Phase 5(표정 AI 문맥 품질 = F2/F3) 프롬프트 대기.** 규칙·Phase 로그·확정 설계는 전부 [`PHASES.md`](./PHASES.md) 에 있다(Claude 계획 → GPT 검토 → 구현 → 검토 → 확정 루프). Phase 프롬프트는 사용자가 하나씩 준다 — **Phase 5 프롬프트를 받기 전까지 Phase 5 를 시작하지 말 것.**
...
- **Phase 4 확정** — 표정 AI 후보가 줄 시점 의상 기준으로 바뀌고(F1) 표정 설명이 후보 identity에서 분리됐다(F4). 구현 `558f18e`, 확정 설계·검증 내역은 `PHASES.md`.
